            ...
            <div className="mb-12 lg:mb-0 lg:flex lg:flex-col">
                ...
                <div className="flex items-center gap-2 mt-auto">
                    {socialLinks.map(({ href, icon }, key)=>(
                        <a href={href} key={key} className="w-12 h-12 grid place-items-center ring-inset ring-2 ring-zinc-50/5 rounded-lg traansition-[background-color,color] hover:bg-zinc-ro hover:text-zinc-950 active:bg-zinc-50/80 reveal-up" target='_blank'>{icon}</a>
                    ))}
                </div>
            </div>

            <form action="https://getform.io/f/awnnelzb" className='xl:pl-10 2xl:pl-20' method="POST">
                <div className="md:grid md:items-center md:grid-cols-2 md:gap-2">
                    <div className="mb-4">
                        <label htmlFor="name" className="label reveal-up">
                            Name
                        </label>
                        <input type="text" name="name" id="name" autoComplete='name' required placeholder='Rapheal Clinton' className="text-field reveal-up" />
                    </div>
                    <div className="mb-4">
                        <label htmlFor="email" className="label reveal-up">
                            Email
                        </label>
                        <input type="email" name="email" id="email" autoComplete='email' required placeholder='rapheal@example.com' className="text-field reveal-up"  />
                    </div>
                </div>
                <div className="mb-4">
                    <label htmlFor="message" className="label reveal-up">Message</label>
                    <textarea name="message" id="message" className='text-field resize-y min-h-32 max-h-80 reveal-up' placeholder='Hi!' required></textarea>
                </div>
